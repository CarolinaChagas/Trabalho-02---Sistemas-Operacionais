...
double calculo_colonia(double P0, double r, double t) {
    return P0 * exp(r * t);  /*retorna a população total de bactérias de cada colonia*/
}
...
void* criacao_colonia(void* arg) {
    ...
    // Colônias pares pegam Recurso A depois B  
    if (colonia->id % 2 == 0) {
        ...
        if (pthread_mutex_trylock(colonia->recursoA) == 0) {
            obteve_recursoA = true;  // Conseguiu o recurso A
            printf("Colonia %d obteve o recurso A\n", colonia->id);
            printf("Colonia %d quer o recurso B\n", colonia->id);
            sleep(1);  // simula o tempo para obter o recurso B
            if (pthread_mutex_trylock(colonia->recursoB) == 0) {
                obteve_recursoB = true;  // Conseguiu o recurso B
            }
        }
    } 

    // Colônias ímpares pegam Recurso B depois A
    else {
        if (pthread_mutex_trylock(colonia->recursoB) == 0) {
            obteve_recursoB = true;  // Conseguiu o recurso B
            printf("Colonia %d obteve o recurso B\n", colonia->id);
            sleep(1);  //simula o tempo para obter o recurso A
            printf("Colonia %d quer recurso A\n", colonia->id);
            if (pthread_mutex_trylock(colonia->recursoA) == 0) {
                obteve_recursoA = true;  // Conseguiu o recurso A
            }
        }
    ...
    // Se conseguiu apenas 1 recurso, imprime que está em impasse
    else if ((obteve_recursoA == true) || (obteve_recursoB == true)) {
        printf("Detectado um impasse!!! \n");
        printf("Colonia %d esta em impasse. Conseguiu apenas 1 recurso e portanto nao podera crescer\n", colonia->id);
    }
...
int main() {
    ...
    for (int i = 0; i < numero_colonias; i++) {
        ...
        colonias[i].recursoB = &recursoB[num_recursos % 1]; /*acessa o recurso 0*/
